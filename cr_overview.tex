...
\begin{document}
  \begin{tikzpicture}[
    ...
    \draw [radio] (Earth)+(160:\Escale * 1.38cm) -- node [midway, above, rotate=-23, yshift=0.1cm] {radio} ([xshift=0.6cm, yshift=-0.45cm]AGN);
    ...
    %% Radio antenna
    \begin{scope}[scale=\Escale]
      \draw [telescope, line join=round] ($(Earth)+(170:1.1cm)$) -- ++(-0.25,0) -- +(0.125,0.2) coordinate(mpoint) -- cycle;
    ...
    %% IACT
    \begin{scope}[scale=\Escale]
      \draw [telescope] ($(Earth)+(130:1cm)$) coordinate (baseR) -- ++(-0.4,0) coordinate (baseL);
      \draw [telescope] ([xshift=0.07cm]baseL) -- +(0.07, 0.15);
      \draw [telescope] ([xshift=-0.07cm]baseR) -- +(-0.07, 0.15);
      \draw [telescope] ([xshift=-0.07cm]baseR) +(-0.07, 0.15) arc(0:180:0.06cm);
      \coordinate (mpoint) at ($(baseR)+(-0.2,0.21)$);
      \begin{scope}
        \draw [telescope] (mpoint) arc(-90:-78:1cm);
        \draw [telescope] (mpoint) arc(-90:-102:1cm);
      \end{scope}
    \end{scope}
\end{tikzpicture}
\end{document}
